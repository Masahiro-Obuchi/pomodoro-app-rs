...
OS間で共有する成功条件は、検証済みの旧本体をバックアップとして保護し、固定候補を同じ保存ディレクトリ内の一時ファイルへ全量書き込んで同期し、本体を一回で置換し、その名前の変更を同期してから成功を返すことである。以下の`fsync`と`rename`はLinux実装の具体的な手順を表す。macOSでは同じ順序でファイルとディレクトリの同期に`F_FULLFSYNC`を使う。Windows実装は同じ順序と失敗時の判定を対応するAPIで満たす。
...
祖先同期の`/`までという範囲はLinuxとmacOSの実装で共有する。Linuxでは`fsync`、macOSでは`rustix::fs::fcntl_fullfsync`を使い、保存先の祖先とrename後の親ディレクトリを同期する。macOSの保存・復旧テストとCIでの実測は[W2検証記録](WINDOWS_MACOS_W2_FINDINGS.md)に置く。Windowsでは書込用ディレクトリハンドルの同期を候補とし、通常保存先のローカルデータ領域で同期範囲の起点と通常ユーザーの権限での成否を実装時に確定する。必要な同期が失敗する場合は保存成功を返さず、固定候補を再試行可能なまま保持する。
...
LinuxとmacOSのローカル保存先を対象に、専用state.lockへ非ブロッキング排他ロックを取得する。ロック取得は本体の読込・初期化判定・復旧より前とし、終了時の最終保存までハンドルを保持する。二つ目の起動は使用中の保存先を示して終了する。

macOSでもLinuxと同じ`rustix::flock`を使い、別プロセスの競合、子プロセスへのロックハンドルの非継承、異常終了後の再取得を検証する。Windowsでは専用ファイルに対するOSのロックを使う予定であり、読込前の取得と異常終了後の再取得を別プロセステストで確認する。
