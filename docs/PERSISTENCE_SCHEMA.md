...
OS間で共有する成功条件は、検証済みの旧本体をバックアップとして保護し、固定候補を同じ保存ディレクトリ内の一時ファイルへ全量書き込んで同期し、本体を一回で置換し、その名前の変更を同期してから成功を返すことである。以下の`fsync`と`rename`はLinux実装の具体的な手順を表す。macOSでは同じ順序でファイルとディレクトリの同期に`F_FULLFSYNC`を使う。Windowsではファイル・ディレクトリハンドルの`sync_all`と同一ディレクトリ内の`std::fs::rename`を使う。
...
保存ディレクトリの作成を永続化する親側の同期も行う。Linux/macOSでは、存在しているディレクトリも以前のプロセスで同期が完了したとは限らないため、各保存ハンドルの最初の保存前に正規化済みの保存先から`/`までの祖先ディレクトリを子側から順に同期する。一つでも失敗した場合は保存成功とせず、ハンドルを開き直した場合も全体を再同期する。同じハンドルで保存成功した後だけ、以降の祖先全体の同期を省略できる。各rename後の親ディレクトリ同期は毎回行う。

祖先同期の`/`までという範囲はLinuxとmacOSの実装で共有する。Linuxでは`fsync`、macOSでは`rustix::fs::fcntl_fullfsync`を使い、保存先の祖先とrename後の親ディレクトリを同期する。macOSの保存・復旧テストとCIでの実測は[W2検証記録](WINDOWS_MACOS_W2_FINDINGS.md)に置く。Windowsでは書込権限を持たない既存のドライブルートやユーザープロファイル祖先を同期対象にしない。各ハンドルの最初の保存では保存ディレクトリを同期し、そのハンドルが新しく作成したディレクトリの親だけを追加で同期する。rename後も保存ディレクトリを同期する。既存の祖先が以前のプロセスで作成されたまま未同期だった場合、そのディレクトリ名の電源断後の存続はこの方式では確認できない。Windows CI runnerでの結果と、Windows 11通常ユーザーで残る確認は[W3検証記録](WINDOWS_MACOS_W3_FINDINGS.md)に置く。必要な同期が失敗する場合は保存成功を返さず、固定候補を再試行可能なまま保持する。
...
Windows/macOSのネイティブプロセス停止テストや同期APIの成功だけで、電源断時にLinuxと同じ永続性が実証されたとは扱わない。macOSの`F_FULLFSYNC`もハードウェアの動作を完全には保証しない。特にWindowsのディレクトリ同期は、対象ファイルシステムと権限で利用できることを実装・受入テストで確かめる。未確認の保証を利用案内へ記載しない。
...
| Windowsで本体rename自体がエラー | 置換済みか判断できないため確定不明とし、本体を再読込してから同じ候補を再試行 |
| 本体rename後、ディレクトリ同期未完了 | 保存の確定状態が不明。本体の再確認を先に行う |
| ディレクトリ同期成功後 | 保存成功 |

Windowsの置換APIがエラーを返した場合は、そのエラーだけで「本体rename前」と決めない。置換の成否が不明な場合は確定不明として本体を再確認し、候補・旧本体のどちらとも一致しなければ競合として止める。
...
初回保存の再試行では、本体が存在しないことに加えて、バックアップや未知の残存物が追加されていないことも再確認する。このハンドルが排他的新規作成した一時ファイルだけは、開いたファイルとの同一性を確認して区別する。同名の外部ファイルやsymlinkへの置換を、自身の一時ファイルとして採用・削除しない。
...
各OSのローカル保存先を対象に、専用state.lockへ非ブロッキング排他ロックを取得する。ロック取得は本体の読込・初期化判定・復旧より前とし、終了時の最終保存までハンドルを保持する。二つ目の起動は使用中の保存先を示して終了する。

macOSでもLinuxと同じ`rustix::flock`を使う。Windowsではリンクを追わずに開いた専用ファイルに`fs4::FileExt::try_lock`を使い、保持中の削除・置換を共有モードで防ぐ。別プロセスの競合、子プロセスへのロックハンドルの非継承、異常終了後の再取得を各OSで検証する。
