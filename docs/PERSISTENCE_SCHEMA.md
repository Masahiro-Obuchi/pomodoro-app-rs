...
保存ディレクトリの作成を永続化する親側の同期も行う。存在しているディレクトリも、以前のプロセスで同期が完了したとは限らない。各保存ハンドルの最初の保存では、本体・backupを書き換える前に、正規化済みの保存先から`/`までの祖先ディレクトリを子側から順に同期する。一つでも失敗した場合は保存成功とせず、ハンドルを開き直した場合も全体を再同期する。同じハンドルで保存成功した後だけ、以降の祖先全体の同期を省略できる。各rename後の親ディレクトリ同期は毎回行う。

祖先同期の`/`までという範囲はLinuxとmacOSの実装で共有する。Linuxでは`fsync`、macOSでは`rustix::fs::fcntl_fullfsync`を使い、保存先の祖先とrename後の親ディレクトリを同期する。macOSの保存・復旧テストとCIでの実測は[W2検証記録](WINDOWS_MACOS_W2_FINDINGS.md)に置く。Windowsでは正規化した保存先からドライブルートまでを対象に、書込用ディレクトリハンドルの`sync_all`を使う。rename後も親ディレクトリを同じ方法で同期する。Windows CI runnerでの結果と、Windows 11通常ユーザーで残る確認は[W3検証記録](WINDOWS_MACOS_W3_FINDINGS.md)に置く。必要な同期が失敗する場合は保存成功を返さず、固定候補を再試行可能なまま保持する。
