...
OS間で共有する成功条件は、検証済みの旧本体をバックアップとして保護し、固定候補を同じ保存ディレクトリ内の一時ファイルへ全量書き込んで同期し、本体を一回で置換し、その名前の変更を同期してから成功を返すことである。以下の`fsync`と`rename`は現行Linux実装の具体的な手順を表す。Windows/macOSの実装は、同じ順序と失敗時の判定を各OSのAPIで満たす。
...
保存ディレクトリの作成を永続化する親側の同期も行う。存在しているディレクトリも、以前のプロセスで同期が完了したとは限らない。各保存ハンドルの最初の保存では、本体・backupを書き換える前に、正規化済みの保存先から`/`までの祖先ディレクトリを子側から順にfsyncする。一つでも失敗した場合は保存成功とせず、ハンドルを開き直した場合も全体を再同期する。同じハンドルで保存成功した後だけ、以降の祖先全体の同期を省略できる。各rename後の親ディレクトリ同期は毎回行う。

この祖先同期の`/`までという範囲はLinuxの現行実装である。macOSではUnixのファイル・ディレクトリ同期を、Windowsでは書込用ディレクトリハンドルの同期を候補とする。Windowsの通常保存先はユーザーのローカルデータ領域に置き、同期範囲の起点と通常ユーザーの権限での成否を実装時に確定する。必要な同期が失敗する場合は保存成功を返さず、固定候補を再試行可能なまま保持する。各OSの実測と未確認事項は[Windows / macOS W0調査](WINDOWS_MACOS_W0_FINDINGS.md)に記録する。
...
Windows/macOSのネイティブプロセス停止テストや同期APIの成功だけで、電源断時にLinuxと同じ永続性が実証されたとは扱わない。特にWindowsのディレクトリ同期は、対象ファイルシステムと権限で利用できることを実装・受入テストで確かめる。未確認の保証を利用案内へ記載しない。
...
Windows/macOSでも同じ排他の開始・終了境界を採用する。現行実装はLinuxの`flock`であり、macOSはUnixのロック、Windowsは専用ファイルに対するOSのロックを使う。ロック取得後に初めて本体を読み、異常終了後は再取得できることを各OSの別プロセステストで確認する。
