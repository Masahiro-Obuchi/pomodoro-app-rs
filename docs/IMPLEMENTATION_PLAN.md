...
現行コードには基本PomodoroのTUI・JSON保存、拡張Domain Modelのコア、V1の読込・通常保存・固定候補の再試行・明示的バックアップ復旧の保存境界、実行時の時刻観測アダプター、UI非依存の保存確定制御がある。新しい起動・終了処理と復旧確認画面・TUI切替は未実装であり、既存TUIは一時的なlegacy APIを使用する。以下は拡張MVPに対する進捗であり、既存機能の詳細台帳ではない。
...
| 2 | V1データを安全に保存・復元できる | Phase 1 | 進行中 | V1の保存・復旧境界、時刻観測、操作・通知を保存後に確定するcontrollerを追加。保存失敗後は固定候補と観測空白の保存が完了するまで操作を保留する。[詳細計画](PHASE2_PLAN.md)に従い起動・復元・終了の接続と既存TUIのV1切替へ進む |
