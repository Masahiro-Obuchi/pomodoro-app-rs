...
現行コードには基本PomodoroのTUI・JSON保存と、拡張Domain Modelのコアがある。V1のファイル保存・復旧・TUI切替は未実装であり、既存TUIは一時的なlegacy APIを使用する。以下は拡張MVPに対する進捗であり、既存機能の詳細台帳ではない。
...
| 2 | V1データを安全に保存・復元できる | Phase 1 | 進行中 | 保存契約・coreの読込検証を補強し、snapshot・historyを含むV1全体codecを追加。[詳細計画](PHASE2_PLAN.md)に従い排他・保存処理・既存TUIのV1切替へ進む |
