...
現行コードには基本PomodoroのTUI・JSON保存、拡張Domain Modelのコア、V1の読込・通常保存の基盤がある。V1の保存再試行・明示的復旧・TUI切替は未実装であり、既存TUIは一時的なlegacy APIを使用する。以下は拡張MVPに対する進捗であり、既存機能の詳細台帳ではない。
...
| 2 | V1データを安全に保存・復元できる | Phase 1 | 進行中 | V1全体codec・排他ロック・読込／書込許可と、通常のatomic saveを追加。保存失敗時は固定候補を保持し、以降の通常保存を停止する。[詳細計画](PHASE2_PLAN.md)に従い再試行・復旧処理・既存TUIのV1切替へ進む |
