...
現行コードには基本PomodoroのTUI・JSON保存、拡張Domain Modelのコア、V1の読込・通常保存・固定候補の再試行・明示的バックアップ復旧の保存境界、実行時の時刻観測アダプターがある。保存確定制御・復旧確認画面・TUI切替は未実装であり、既存TUIは一時的なlegacy APIを使用する。以下は拡張MVPに対する進捗であり、既存機能の詳細台帳ではない。
...
| 2 | V1データを安全に保存・復元できる | Phase 1 | 進行中 | V1全体codec・排他ロック・読込／書込許可、atomic save・再試行・明示的復旧の保存境界、UTCと単調時計による時刻観測を追加。[詳細計画](PHASE2_PLAN.md)に従い保存確定制御・既存TUIのV1切替へ進む |
