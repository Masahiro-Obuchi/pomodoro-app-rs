...
通常実行のTUIはV1の保存・復元経路を使用する。拡張Domain Model、固定候補の保存と再試行、実行時の時刻観測、保存後の通知を接続し、起動時の明示的バックアップ復旧確認と、起動・終了時の保存失敗への操作経路を用意した。既存キー操作、Readyでの設定変更、復元した全状態の最小表示と操作が利用できる。一時的なlegacy APIの撤去とPhase 2全体の受入確認が残る。以下は拡張MVPに対する進捗であり、既存機能の詳細台帳ではない。
...
| 2 | V1データを安全に保存・復元できる | Phase 1 | 進行中 | 復旧確認画面を含む通常起動のV1切替まで実装。実行ファイルの保存・再起動・二重起動拒否・未保存終了と、保存境界での子プロセス停止を検証。[詳細計画](PHASE2_PLAN.md)に従い、legacy撤去とPhase受入確認へ進む |
