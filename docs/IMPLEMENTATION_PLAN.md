...
現行コードには基本PomodoroのTUI・JSON保存、拡張Domain Modelのコア、V1の読込・通常保存・固定候補の再試行・明示的バックアップ復旧の保存境界、実行時の時刻観測アダプター、UI非依存の保存確定制御と起動・復元・終了処理がある。復旧確認画面を含む既存TUIへの接続と通常起動経路の切替は未実装であり、既存TUIは一時的なlegacy APIを使用する。以下は拡張MVPに対する進捗であり、既存機能の詳細台帳ではない。
...
| 2 | V1データを安全に保存・復元できる | Phase 1 | 進行中 | V1の保存・復旧境界とcontrollerに、起動時の復元保存、明示復旧、終了保存と未保存終了の区別を接続。失敗時は固定候補を再試行し、通常操作の保存回復では観測空白の保存まで操作を保留する。[詳細計画](PHASE2_PLAN.md)に従い既存TUIの表示・操作の接続と通常起動のV1切替へ進む |
