...
現行コードには基本PomodoroのTUI・JSON保存と、拡張Domain Modelのコアがある。Phase 2は保存契約の補足と詳細計画の文書化から着手している。V1保存と新しいユーザーフローのTUI接続のコード実装は未着手であり、既存TUIは一時的なlegacy APIを使用する。以下は拡張MVPに対する進捗であり、既存機能の詳細台帳ではない。
...
| 2 | V1データを安全に保存・復元できる | Phase 1 | 進行中 | 保存契約を補足。[詳細計画](PHASE2_PLAN.md)の単位で永続化を検証し、既存TUIのV1切替まで行う |
