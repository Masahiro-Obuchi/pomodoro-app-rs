...
これらを満たす安全性を監査した実装手段が見つかるまでは、その OS の保存層を「対応済み」としない。W3ではReFSの所有判定に必要な128-bit file IDをRust 1.86の標準APIから取得できないため、専用の小さなFFI crateにWin32呼出しを隔離し、platform本体の`unsafe_code = "forbid"`を維持する。特に電源断耐性を既存仕様と同じ水準で示せない場合は、実装・README を先行して対応済みにせず、保証の差と代替案を Persistence Schema に明記する変更を別途レビューする。プロセスを強制終了するテストは電源断の証明とは扱わない。
